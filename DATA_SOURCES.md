...
- **Subset used** (documented explicitly here and in `modules/1_cellpainting/README.md` because it is a deliberate compute-driven substitution, not the full dataset): a small number of compounds and a single plate from one imaging batch, to validate the pipeline end-to-end before scaling up. Exact plate ID, compound list, and image/profile counts are recorded in `modules/1_cellpainting/README.md` once the download script has run, so the record reflects what was actually pulled rather than what was planned.
- Precomputed CellProfiler feature profiles (Broad's public parquet exports) are used for the wider-scale UMAP/SHAP baseline, since these are orders of magnitude smaller than raw images.
