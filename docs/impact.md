...
Planned claim shape (to be replaced with real numbers once the batch-transfer experiment runs): in-distribution compound-identity classification accuracy/macro-F1 (14-way — no MoA annotation exists for this pilot's compound set, see `docs/build-log.md`) vs. cross-batch transfer accuracy/macro-F1, and the point-drop between them, compared against a naive (e.g. majority-class or non-deep tabular) baseline.
