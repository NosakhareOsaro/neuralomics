...
A CNN trained directly on raw 5-channel Cell Painting pixel composites (ch1–ch5: Mito/AGP/RNA/ER/DNA, per the confirmed channel mapping above) to classify compound identity (14-way, not mechanism-of-action — see Research question above) — not on CellProfiler features. The CNN never runs CellProfiler or consumes its output; Broad's precomputed profiles (see Feature extraction above) feed only the naive/tabular baseline and the UMAP/SHAP exploratory analysis this module is compared against.
...
**CNN's in-distribution split**: 4-fold leave-one-plate-out across the same 4 verified plates, with a rotating third plate held out purely for early stopping (never contributing to the reported metric) so the actual test plate isn't touched during training — decided in `docs/build-log.md` before any training code was written, since a fine-tuned CNN needs an internal validation set the tabular baseline's logistic regression didn't. All splits are at plate level, never site/image level within a plate, to avoid leaking well/plate-specific imaging conditions across the split. Reported in-distribution accuracy/macro-F1 pools all 504 out-of-fold test predictions across the 4 folds (every site composite is a test prediction exactly once), the same out-of-fold-pooling convention as the tabular baseline's `evaluate()`. Significance is checked two ways, both adapted for a fine-tuned CNN's compute cost rather than the tabular baseline's 1000-permutation test (infeasible here — each "permutation" would mean refitting the full 4-fold CNN pipeline, not a few-second logistic regression fit): a closed-form binomial test of the pooled predictions against chance ≈ 1/14 (justified by the tabular baseline's own permutation run already showing this design's true null lands at 0.0717, almost exactly 1/14 — not an unexamined assumption), plus a small number of full label-shuffled retraining runs as a pipeline-correctness sanity check, not a rigorous p-value.
